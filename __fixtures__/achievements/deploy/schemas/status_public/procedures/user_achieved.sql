...
BEGIN;
CREATE FUNCTION status_public.user_achieved (vlevel text, vrole_id uuid DEFAULT jwt_public.current_user_id ())
  RETURNS boolean
  AS $$
DECLARE
  c int;
BEGIN
  SELECT
    COUNT(*)
  FROM
    status_public.steps_required (vlevel, vrole_id) INTO c;
  RETURN c <= 0;
END;
$$
LANGUAGE 'plpgsql'
