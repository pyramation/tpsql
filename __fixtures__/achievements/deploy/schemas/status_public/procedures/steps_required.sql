...
-- ;
CREATE FUNCTION status_public.steps_required (vlevel text, vrole_id uuid DEFAULT jwt_public.current_user_id ())
...
BEGIN
  ...
  FROM
    status_public.level_requirements
  FULL OUTER JOIN status_public.user_achievements ON (user_achievements.name = level_requirements.name
