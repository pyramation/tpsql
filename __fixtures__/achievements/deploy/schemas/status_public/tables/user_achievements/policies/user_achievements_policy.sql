...
BEGIN;
CREATE POLICY can_select_user_achievements ON status_public.user_achievements FOR SELECT USING (jwt_public.current_user_id () = user_id);
CREATE POLICY can_insert_user_achievements ON status_public.user_achievements FOR INSERT WITH CHECK (FALSE);
CREATE POLICY can_update_user_achievements ON status_public.user_achievements FOR UPDATE USING (FALSE);
CREATE POLICY can_delete_user_achievements ON status_public.user_achievements FOR DELETE USING (FALSE);
